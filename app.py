...
def get_highest_KA_cases_data():
    highest_KA_cases = df.groupby('Year')['K&A'].max()
    trace = go.Bar(x=highest_KA_cases.index, y=highest_KA_cases.values, name='Highest K&A Cases Each Year')
    return json.dumps([trace], cls=plotly.utils.PlotlyJSONEncoder)
...
def yearly_comparison():
    ...
    highest_KA_cases_data = get_highest_KA_cases_data()
    ...
    return render_template('yearly_comparison.html', highest_lowest_other_crimes_data=highest_lowest_other_crimes_data,
                           ...
                           highest_KA_cases_data=highest_KA_cases_data,
